...
function EditProjectForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${id}`)
      ...
      .catch(() => { setError('Failed to load project'); setFetching(false) })
  }, [id])
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{
        ...
        <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.1rem' }}>
          Mercy<span style={{ color: 'var(--teal)' }}>.</span>Dev <span style={{ color: 'var(--text3)', fontWeight: 400, fontSize: '0.85rem' }}>/ Edit Project</span>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <div>
            ...
            <textarea name="description" value={form.description} onChange={handleChange} required rows={4}
              placeholder="Describe what you built..." style={{ ...inputStyle, resize: 'vertical' }} />
          </div>
          ...
          <div>
            ...
            <input name="imageUrl" value={form.imageUrl} onChange={handleChange} required
              placeholder="https://your-image-url.com/screenshot.png" style={inputStyle} />
            <p style={{ color: 'var(--text3)', fontSize: '0.75rem', marginTop: '0.3rem' }}>
              Upload to <a href="https://cloudinary.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--teal)' }}>cloudinary.com</a> and paste the URL
            </p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              ...
              <input name="liveUrl" value={form.liveUrl} onChange={handleChange} placeholder="https://yourproject.com" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>GitHub URL</label>
              <input name="githubUrl" value={form.githubUrl} onChange={handleChange} placeholder="https://github.com/you/repo" style={inputStyle} />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '2rem' }}>
            {[
              ...
              <label key={toggle.name} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.88rem', color: 'var(--text2)' }}>
                <input type="checkbox" name={toggle.name} checked={toggle.checked} onChange={handleChange}
